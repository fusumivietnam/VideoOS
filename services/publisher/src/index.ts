...
export class FakeNetworkPublisherAdapter implements NetworkPublisherAdapter {
  readonly network: SocialNetwork;
  private readonly status: PublishReceipt["status"];
  private readonly externalBaseUrl: string;

  constructor(options: FakePublisherOptions = {}) {
    this.network = options.network ?? "youtube";
    this.status = options.status ?? "published";
    this.externalBaseUrl = options.externalBaseUrl ?? "https://fake.videoos.local/posts";
  }

  async publish(request: PublishRequest): Promise<PublishReceipt[]> {
    const targets = request.targets.filter((target) => target.network === this.network);
    return targets.map((target, index) => {
      const externalPostId = `fake:${this.network}:${request.idempotencyKey}:${index + 1}`;
      return {
        target,
        externalPostId,
        externalUrl: `${this.externalBaseUrl}/${encodeURIComponent(externalPostId)}`,
        status: this.status,
        providerCode: "fake_provider",
        message: "Synthetic receipt generated by VideoOS fake publisher",
      };
    });
  }
}
