...
CI run #92 proved the gate end-to-end after correcting the PostgreSQL 18 volume mount to `/var/lib/postgresql`.

## Acceptance Criteria
- [x] #1 The existing CI workflow detects relevant persistence/runtime path changes without adding a second workflow.
- [x] #2 PostgreSQL is started only for relevant pull-request changes or explicit manual execution.
- [x] #3 Migrations run before integration assertions.
- [x] #4 Integration coverage includes leasing, final-attempt exhaustion, outbox behavior, transaction rollback, and atomic queue settlement.
- [x] #5 Unrelated documentation/UI/provider-only pull requests do not pay PostgreSQL startup cost.
- [x] #6 CI permissions remain read-only unless a narrowly documented write capability is required.
